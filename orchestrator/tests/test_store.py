...
class TestRetention:
    def test_prune_drops_only_old_rows(self, store: Store) -> None:
    ...
    def test_prune_disabled_at_zero_days(self, store: Store) -> None:
        store.record_event(EVENT_SPAWN, pool="a")
...
class TestAudit:
    def test_audit_rows_are_ordered_newest_first(self, store: Store) -> None:
...
class TestJobs:
    ...
    def test_new_job_is_reported_as_started(self, store: Store) -> None:
        started, finished = store.sync_jobs("a", [self._JOB])

        assert [j["job_id"] for j in started] == [101]
        assert finished == []

    def test_same_job_is_not_reported_twice(self, store: Store) -> None:
    ...
    def test_completion_is_reported_once(self, store: Store) -> None:
        store.sync_jobs("a", [self._JOB])
    ...
    def test_running_job_is_attributed_to_its_runner(self, store: Store) -> None:
        store.sync_jobs("a", [self._JOB])
    ...
    def test_other_pools_are_untouched(self, store: Store) -> None:
    ...
    def test_recent_jobs_newest_first(self, store: Store) -> None:
        store.sync_jobs("a", [self._JOB])
        store.sync_jobs(
